...
import (
	...
	"gitlab.zhonganonline.com/ann/ann-module/xlib"
)
...
func (asm *authSigMessage) writeBytes(writer io.Writer) (err error) {
	if err = xlib.WriteBytes(writer, asm.Key.Bytes()); err != nil {
		return
	}
	err = xlib.WriteBytes(writer, asm.Sig.Bytes())
	return
}

func (asm *authSigMessage) readBytes(reader io.Reader) (err error) {
	var keyBys, sigBys []byte
	if keyBys, err = xlib.ReadBytes(reader); err != nil {
		return
	}
	if len(keyBys) < 32 {
		return errors.New("not enough bytes for key")
	}
	var bys32 [32]byte
	copy(bys32[:], keyBys)
	key := crypto.PubKeyEd25519(bys32)
	asm.Key = &key
	if sigBys, err = xlib.ReadBytes(reader); err != nil {
		return
	}
	if len(sigBys) < 64 {
		return errors.New("not enough bytes for sig")
	}
	var bys64 [64]byte
	copy(bys64[:], sigBys)
	sig := crypto.SignatureEd25519(bys64)
	asm.Sig = &sig
	return
}
