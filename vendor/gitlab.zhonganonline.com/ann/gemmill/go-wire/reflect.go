...
func GetTypeFromStructDeclaration(o interface{}) reflect.Type {
	rt := reflect.TypeOf(o)
	if rt.NumField() != 1 {
		gcmn.PanicSanity("Unexpected number of fields in struct-wrapped declaration of type")
	}
...
func RegisterInterface(o interface{}, ctypes ...ConcreteType) *TypeInfo {
	...
	if it.Kind() != reflect.Interface {
		gcmn.PanicSanity("RegisterInterface expects an interface")
	}
	...
	for _, ctype := range ctypes {
		...
		typeByte := ctype.Byte
		if typeByte == 0x00 {
			gcmn.PanicSanity(gcmn.Fmt("Byte of 0x00 is reserved for nil (%v)", ctype))
		}
		if toType[typeByte] != nil {
			gcmn.PanicSanity(gcmn.Fmt("Duplicate Byte for type %v and %v", ctype, toType[typeByte]))
		}
		toType[typeByte] = crt
		toByte[crt] = typeByte
	}
...
func readReflectBinary(rv reflect.Value, rt reflect.Type, opts Options, r io.Reader, lmt int, n *int, err *error) {
	...
	if rt.Kind() == reflect.Interface {
		if !typeInfo.IsRegisteredInterface {
			...
			*err = errors.New(gcmn.Fmt("Cannot read unregistered interface type %v", rt))
			return
		}
		...
		if !ok {
			*err = errors.New(gcmn.Fmt("Unexpected type byte %X for type %v", typeByte, rt))
			return
		}
	...
	if rt.Kind() == reflect.Ptr {
		...
		if typeByte == 0x00 {
			...
			*err = errors.New(gcmn.Fmt("Unexpected type byte %X for ptr of untyped thing", typeByte))
			return
		}
	...
	case reflect.Slice:
		...
		if elemRt.Kind() == reflect.Uint8 {
			...
			for i := 0; i*ReadSliceChunkSize < length; i++ {
				l := gcmn.MinInt(ReadSliceChunkSize, length-i*ReadSliceChunkSize)
	...
	default:
		gcmn.PanicSanity(gcmn.Fmt("Unknown field type %v", rt.Kind()))
	}
}
	...
	if rt.Kind() == reflect.Interface {
		...
		if typeInfo.IsRegisteredInterface {
			...
			if !ok {
				switch crt.Kind() {
				case reflect.Ptr:
					*err = errors.New(gcmn.Fmt("Unexpected pointer type %v for registered interface %v. "+
						"Was it registered as a value receiver rather than as a pointer receiver?", crt, rt.Name()))
				case reflect.Struct:
					*err = errors.New(gcmn.Fmt("Unexpected struct type %v for registered interface %v. "+
						"Was it registered as a pointer receiver rather than as a value receiver?", crt, rt.Name()))
				default:
					*err = errors.New(gcmn.Fmt("Unexpected type %v for registered interface %v. "+
			...
			if crt.Kind() == reflect.Ptr {
				...
				if !crv.IsValid() {
					*err = errors.New(gcmn.Fmt("Unexpected nil-pointer of type %v for registered interface %v. "+
...
func readByteJSON(o interface{}) (typeByte byte, rest interface{}, err error) {
	...
	if !ok {
		err = errors.New(gcmn.Fmt("Expected type [Byte,?] but got type %v", reflect.TypeOf(o)))
		return
	}
	if len(oSlice) != 2 {
		err = errors.New(gcmn.Fmt("Expected [Byte,?] len 2 but got len %v", len(oSlice)))
		return
	}
...
func readReflectJSON(rv reflect.Value, rt reflect.Type, opts Options, o interface{}, err *error) {
	...
	if rt.Kind() == reflect.Interface {
		...
		crt, ok := typeInfo.ByteToType[typeByte]
		if !ok {
			*err = errors.New(gcmn.Fmt("Byte %X not registered for interface %v", typeByte, rt))
			return
		}
		if crt.Kind() == reflect.Ptr {
			crt = crt.Elem()
	...
	case reflect.Array:
		...
		if elemRt.Kind() == reflect.Uint8 {
			...
			oString, ok := o.(string)
			if !ok {
				*err = errors.New(gcmn.Fmt("Expected string but got type %v", reflect.TypeOf(o)))
				return
			}
			...
			if len(buf) != length {
				*err = errors.New(gcmn.Fmt("Expected bytearray of length %v but got %v", length, len(buf)))
				return
			}
			...
			oSlice, ok := o.([]interface{})
			if !ok {
				*err = errors.New(gcmn.Fmt("Expected array of %v but got type %v", rt, reflect.TypeOf(o)))
				return
			}
			if len(oSlice) != length {
				*err = errors.New(gcmn.Fmt("Expected array of length %v but got %v", length, len(oSlice)))
				return
			}
			for i := 0; i < length; i++ {
				elemRv := rv.Index(i)
	...
	case reflect.Struct:
		if rt == timeType {
			...
			if typeInfo.Unwrap {
				...
				if !ok {
					*err = errors.New(gcmn.Fmt("Expected map but got type %v", reflect.TypeOf(o)))
					return
				}
	...
	case reflect.Int64, reflect.Int32, reflect.Int16, reflect.Int8, reflect.Int:
		num, ok := o.(float64)
		if !ok {
			*err = errors.New(gcmn.Fmt("Expected numeric but got type %v", reflect.TypeOf(o)))
			return
		}
	...
	case reflect.Uint64, reflect.Uint32, reflect.Uint16, reflect.Uint8, reflect.Uint:
		...
		if num < 0 {
			*err = errors.New(gcmn.Fmt("Expected unsigned numeric but got %v", num))
			return
		}
		//log.Info("Read num", "num", num)
		rv.SetUint(uint64(num))
	...
	case reflect.Bool:
		bl, ok := o.(bool)
		if !ok {
			*err = errors.New(gcmn.Fmt("Expected boolean but got type %v", reflect.TypeOf(o)))
			return
		}
...
func writeReflectJSON(rv reflect.Value, rt reflect.Type, opts Options, w io.Writer, n *int, err *error) {
	//log.Info(gcmn.Fmt("writeReflectJSON(%v, %v, %v, %v, %v)", rv, rt, w, n, err))
	...
	if rt.Kind() == reflect.Interface {
		...
		if typeInfo.IsRegisteredInterface {
			...
			WriteTo([]byte(gcmn.Fmt("[%v,", typeByte)), w, n, err)
	...
	case reflect.Array:
		...
		if elemRt.Kind() == reflect.Uint8 {
			...
			WriteTo([]byte(gcmn.Fmt("\"%X\"", bytearray.Interface())), w, n, err)
		} else {
			WriteTo([]byte("["), w, n, err)
			// Write elems
			for i := 0; i < length; i++ {
	...
	case reflect.Slice:
		...
		if elemRt.Kind() == reflect.Uint8 {
			// Special case: Byteslices
			byteslice := rv.Bytes()
			WriteTo([]byte(gcmn.Fmt("\"%X\"", byteslice)), w, n, err)
		} else {
			WriteTo([]byte("["), w, n, err)
			// Write elems
			length := rv.Len()
		...
		if rt == timeType {
			...
			if typeInfo.Unwrap {
				...
				for _, fieldInfo := range typeInfo.Fields {
					...
					WriteTo([]byte(gcmn.Fmt("\"%v\":", opts.JSONName)), w, n, err)
					writeReflectJSON(fieldRv, fieldType, opts, w, n, err)
				}
				WriteTo([]byte("}"), w, n, err)
			}
